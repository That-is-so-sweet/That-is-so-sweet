...
export const GoogleLoginOverlay: React.FC = () => {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,18,8,0.55)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ width: "100%", maxWidth: 300, background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "var(--shadow-lg)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", borderBottom: "1px solid #e8eaed", background: "#f1f3f4" }}>
          <span style={{ display: "flex", gap: 3, flexShrink: 0 }}>
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#ff5f57" }} />
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#febc2e" }} />
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#28c840" }} />
          </span>
          <div style={{ flex: 1, background: "#fff", borderRadius: 999, padding: "3px 8px", fontSize: 10, color: "#5f6368", display: "flex", alignItems: "center", gap: 4, overflow: "hidden" }}>
            🔒 accounts.google.com
          </div>
        </div>
        <div style={{ padding: "28px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: "#3c4043" }}>正在完成 Google 登入…</div>
          <div style={{ fontSize: 10, color: "#80868b" }}>（Demo 模擬跳轉畫面，不會連線至 Google）</div>
        </div>
      </div>
    </div>
  );
